...
Note the following example: `/root/Desktop/my_images/cat.jpg`
...
- The `cat.jpg` image file is located in the `my_images` directory, which is in the `Desktop` directory, which is in the `root` directory.
 ...
 File paths are used by IT and security professionals to describe where files or other directories are located.
  ...
  - For example, if you are a security administrator and need to access a network log for an investigation, you may need to ask the network team for the location or path of the log file. They would give you a path like `/root/var/logfile1.txt`.
  
    - This path says the `logfile1.txt` log file is in the `var` directory, which is in the `root` directory.
...
- When writing an absolute path, we need to include the `root` directory. We will discuss the root directory in more detail in later units, but for now, think of it as the highest level of a file structure.

- For example, we want to access a file located at `/root/Desktop/Sallysfiles/textfiles/Sallyfile.txt`.

  - If we are located in another directory, such as `/root/rootfiles/file_logs/`, we can use an absolute path of `/root/Desktop/Sallysfiles/textfiles/Sallyfile.txt` to access `Sallyfile.txt`.

 A relative path, on the other hand, starts at your current location in the file directory. It doesn't require writing out the root directory.
...
- For example, to access `Sallyfile.text`, we can use `/Sallysfiles/textfiles/Sallyfile.txt`.
  ...
  - We are already in the `Desktop` directory, therefore `root/Desktop/` is assumed, and we don't have to write it. 
...
Relative and absolute paths are very helpful shortcuts when working on the command line. 
  ...
  - For example: Let's say you want to access the `Sallysfiles` directory from the previous example, starting at the root directory.
  
    - Instead of running the following three commands from the root directory:
      - `cd Desktop`
      - `cd textfiles`
      - `cd Sallysfiles`
   
       You can use the relative path from the root directory as a single command:
...
2.  Next, your manager requested you to navigate to the `security_evidence/Case2/` directory.
     ...
     - To navigate to that directory using a relative path, you would run:
        ...
        * You have to be in the root directory to use this relative path.
...
4. Lastly, you have been asked to return to the root directory.
    ...
    - To go back to root from our current location, we need to go back four directories.
    
      - Run the command `cd ../../../../`.
    
    - Run `pwd` again to show that you are back in the `/root/` directory.
...
1. Similar to the previous demonstrations, you need to know your current location in the file directory to get started.
    ...
    - Run the `pwd` command to retrieve the path of your current location. 
      ...
      - Your current location should display as `/root`.

        Change back to root if you are not currently there.
...
In the previous two demonstrations, we practiced the following:
   ...
   - `absolute paths`: A path that includes the root of the file system.
...
- In order to save the results of `cat`, the user would have to use either:
  ...
  For example: You want to combine two separate files, `1.txt` and `2.txt`, into a single file called `combined_1&2.txt`.

  - You would run the following command:

    `cat 1.txt 2.txt > combined_1&2.txt`

  - This command will create a new file called `combined_1&2.txt`. If the file name already exists, it will overwrite it with the combined data from `1.txt` and `2.txt`.
    
  Another example: You want to combine two separate files, `3.txt` and `4.txt`, into a single file called `combined_3&4.txt`.
  
  - You would run the following command:

    `cat 3.txt 4.txt >> combined_3&4.txt` 

  - This command will create a new file called `combined_3&4.txt`. If the file name already exists, it will add to the bottom of the file with the combined data from `3.txt` and `4.txt`.
...
- _stdout_ is then redirected and written to a file called `combined_1&2.txt`.
